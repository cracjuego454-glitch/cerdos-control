    ...
    return `
      ...
      <div class="card">
        <div class="table-container">
          <table>
            <tr><th>Fecha</th><th>Categoría</th><th>Descripción</th><th>Cerdo</th><th>Monto</th><th></th></tr>
            ${records.length ? records.map(r => `
              <tr>
                <td>${r.date}</td><td>${cats[r.category] || r.category}</td><td>${r.description || '-'}</td>
                <td>${r.pig_identifier || '-'}</td><td><strong>$${r.amount.toFixed(2)}</strong></td>
                <td><button class="btn btn-sm btn-danger" onclick="Expenses.delete(${r.id})">🗑️</button></td>
              </tr>
            `).join('') : '<tr><td colspan="6" class="empty"><p>No hay gastos registrados</p></td></tr>'}
          </table>
        </div>
      </div>
